...
read -p "Confirm to ${TYPE}ly disable $NODE for $ES? [y/N]" c
[ "$c" = y -o "$c" = Y ] || exit 1
...
if perl -e 'exit(scalar(grep $_ eq $ARGV[1], split(/\s*,\s*/, $ARGV[0])) == 0 ? 0 : 1)' "$nodes" "$NODE"; then
    nodes=`perl -e '%h = map { $_, 1 } split(/\s*,\s*/, $ARGV[0]), $ARGV[1]; print join(",", sort keys %h)' "$nodes" "$NODE"`
    ...
    echo "$NODE is already in excluding list."
fi
...
echo "then periodically use these commands to check if $NODE is really decommissioned:"
